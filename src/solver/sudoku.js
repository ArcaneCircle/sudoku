...
sudoku.solve = function (board, reverse) {
  ...
  if (result) {
    let solution = ''
    for (const square in result) solution += result[square]
...
sudoku.get_candidates = function (board) {
  ...
  const candidates_map = sudoku._get_candidates_map(board)
...
sudoku._get_candidates_map = function (board) {
  ...
  for (const si in SQUARES) candidate_map[SQUARES[si]] = sudoku.DIGITS
  ...
  for (const square in squares_values_map) {
    ...
    if (sudoku._in(val, sudoku.DIGITS)) {
      const new_candidates = sudoku._assign(candidate_map, square, val)
...
sudoku._assign = function (candidates, square, val) {
  ...
  for (const ovi in other_vals) {
    ...
    const candidates_next = sudoku._eliminate(candidates, square, other_val)
...
sudoku._get_square_vals_map = function (board) {
  ...
  if (board.length !== SQUARES.length) {
    ...
    for (const i in SQUARES) squares_vals_map[SQUARES[i]] = board[i]
  }
...
sudoku._get_all_units = function (rows, cols) {
  ...
  // Rows
  for (const ri in rows) units.push(sudoku._cross(rows[ri], cols))

  // Columns
  for (const ci in cols) units.push(sudoku._cross(rows, cols[ci]))
...
sudoku.board_grid_to_string = function (board_grid) {
  ...
  let board_string = ''
  for (let r = 0; r < 9; ++r)
    for (let c = 0; c < 9; ++c) board_string += board_grid[r][c]
...
sudoku._cross = function (a, b) {
  ...
  const result = []
  for (const ai in a)
    for (const bi in b) result.push(a[ai] + b[bi])
